...
regp_number = 100
reg_param_grid = \
    np.round(np.logspace(regp_min, regp_max, regp_number), 3)[::-1]
...
centered_points = reg_path_0 \
    - np.mean(reg_path_0, axis=1).reshape(reg_path_0.shape[0], 1)
...
def reg_param_selection(path, ground_truth):
    ...
    for k in np.arange(path_length):
        ...
        if la.norm(ground_truth.T - path[:, k]) \
                < la.norm(ground_truth.T - reg_sol):
